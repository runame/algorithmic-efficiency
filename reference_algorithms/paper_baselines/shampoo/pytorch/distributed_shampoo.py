...
# A list of the total numbers of parameter elements per order
# (i.e., tensor dimension)
...
# A list of total sizes of local distributed buffer size allocated with each
# rank (bytes)
...
class DistributedShampoo(torch.optim.Optimizer):
  """Implements distributed Shampoo algorithm.
    ...
    Rohan Anil (Google), Adnan Aziz (Meta), Pavan Balaji (Meta), Shuo Chang
    (Meta), Weiwei Chu (Meta), Assaf Eisenman (Meta), Will Feng (Meta), Zhuobo
    Feng (Meta), Jose Gallego-Posada (Mila / Meta Platforms, Inc.), Avirup Ghosh
    (Meta), Yizi Gu (Meta), Vineet Gupta (Google), Yuchen Hao (Meta), Yusuo Hu
    (Meta), Yuxi Hu (Meta), Minhui Huang (Meta), Guna Lakshminarayanan (Meta),
    Zhijing Li (Meta), Ming Liang (Meta), Wanchao Liang (Meta), Ying Liu (Meta),
    Wenguang Mao (Meta), Dheevatsa Mudigere (NVIDIA), Maxim Naumov (Meta),
    Jongsoo Park (Meta), Mike Rabbat (Meta), Kaushik Rangadurai (Meta), Ke Sang
    (Meta), Dennis van der Staay (Meta), Fei Tian (Meta), Sanjay Vishwakarma
    (Meta), Xunnan (Shawn) Xu (Meta), Jiyan Yang (Meta), Chunxing Yin (Meta),
    Iris Zhang (Meta), and Wang Zhou (Meta).
    ...
    Uses infinity norm to evaluate residuals and errors. By default, grafts from
    Adagrad.
    ...
    If one wants to use DTensor which leads to memory savings, please set
    use_dtensor = True. Requires PyTorch 2 nightly build.

    In order to support checkpointing, one must use torch.distributed.checkpoint
    and pass the named parameters into state_dict.
    Note that the standard checkpointing solution by PyTorch is not supported!

    Note: We have observed known instabilities with the torch.linalg.eigh
    operator on CUDA 11.6-12.1, specifically for low-rank
    matrices, which may appear with using a small start_preconditioning_step.
    Please avoid these versions of CUDA if possible.
    ...
    1. Layerwise Grafting: In order to tune Shampoo, we can "graft" a layer-wise
    learning rate schedule from a previous method
        and apply it to Shampoo. This is performed by taking the norm of the
        layer-wise step of the grafted method, normalizing
        the Shampoo step, and re-scaling the normalized Shampoo step by the
        product of the norm of the grafted step + learning rate.

        This may be interpreted as an additional block re-scaling of the entire
        ...
        We support the following methods:
            ...
            - GraftingType.ADAGRAD_NORMALIZED: Grafts the Adagrad method with
            normalized gradients.
            - GraftingType.RMSPROP_NORMALIZED: Grafts the RMSProp method with
            normalized gradients.
            - GraftingType.ADAM_NORMALIZED: Grafts the Adam method with
            normalized gradients.
            - GraftingType.LARS: Grafts the LARS method.
            - GraftingType.LAMB: Grafts the LAMB method.

        NOTE: These methods do not graft the first-moment component - it is
        entirely based upon grafting using the
        diagonal preconditioner. If using an exponential moving average of the
        gradient (or gradient filtering), we
        can set beta1 as the same value from before, and both Shampoo and the
        grafted method will use the filtered
        gradient.

    2. Large-Dimensional Tensors: Supports multiple approaches for scaling
        Shampoo to tensors with large dimensions.
        For simplicity, we explain using a linear layer/matrix parameter,
        although this is generalizable to higher-order tensors.
        ...
        - LargeDimMethod.BLOCKING (Default): Given a max_preconditioner_dim tau
            > 0, blocks W and applies Shampoo to each block, i.e., if tau
            divides both m, n, then:
            ...
            and apply Shampoo to W_ij which is a tau x tau matrix. This can be
            viewed as further blocking each block of the block-diagonal
            preconditioner.

            Computational cost = O(tau^3)
            Memory cost = 4mn (including root inverse preconditioners)

        - LargeDimMethod.ADAGRAD: Given a max_preconditioner_dim tau > 0, checks 
            if any dimensions of the tensor is greater than tau. If so, uses
            Adagrad preconditioner in place of Shampoo. Corresponds to a
            diagonal preconditioner.

            Computational cost = O(mn)
            Memory cost = mn

        - LargeDimMethod.DIAGONAL: Given a max_preconditioner_dim tau > 0, uses
            a diagonal Shampoo preconditioner in place of the full Shampoo
            preconditioner. Corresponds to a (crude) diagonal preconditioner.

            Computational cost = O(mn)
            Memory cost = m + n

    3. Distributed Memory and Computation: Supports multi-GPU data-parallel
        training via torch.distributed by setting num_trainers_per_group > 1.
        Distributes the computation required for Shampoo (updating of the
        preconditioners, computation of the root inverse, preconditioning of the
        gradients, etc.) across multiple GPUs. The memory is similarly
        distributed using DTensor. num_trainers_per_group specifies the number
        of GPUs used per distributed group. The computation is replicated across
        different groups.
    ...
    Args:
        ...
        betas (Tuple[float, float]): coefficients used for computing running
            averages of gradient and its square (Default: (0.9, 1.0))
        epsilon (float): term added to the denominator to improve numerical
            stability (Default: 1e-12)
        momentum (float): momentum parameter (default: 0.)
        weight_decay (float): weight decay (L2 penalty) (Default: 0)
        max_preconditioner_dim (int): maximum preconditioner dimension
            (Default: 1024)
        precondition_frequency (int): frequency for computing root inverse
            preconditioner (Default: 1)
        start_preconditioning_step (int): iteration to start computing inverse
            preconditioner. If -1, uses the same value as
            precondition_frequency. (Default: -1)
        exponent_override (int, List[int]): inverse root to use in Shampoo.
            If a list [l1, l2, ..., lp], then we will use -1 / l1 for 1-D tensor
            (vectors), -1 / l2 for 2-D tensors (matrices), and so on. If the
            order of the tensor exceeds the order of the tensor, reverts to the
        ...
        use_bias_correction (bool): flag for using bias correction
            (Default: True)
        use_decoupled_weight_decay (bool): Flag for using AdamW-style decoupled
            weight decay (Default: True)
        grafting_type (GraftingType): selects grafting method.
            (Default: GraftingType.ADAGRAD)
        grafting_epsilon (float): epsilon for grafting method.
            (Default: 1e-3)
        grafting_beta2 (float): exponential moving average factor for grafting
            method. (Default: 1.0)
        large_dim_method (LargeDimMethod): method for handling large scale
            tensors. (Default: LargeDimMethod.BLOCKING)
        use_merge_dims (bool): merge dimensions if possible while respecting
            max_preconditioner_dim. (Default: True)
        preconditioner_dtype (torch.dtype): data type for preconditioner
            (Default: torch.float)
        communication_dtype (CommunicationDType): Datatype for communication
            between ranks. (Default: DEFAULT)
        num_trainers_per_group (int): number of GPUs per distributed process
            group for distributed computation/memory.
            If num_trainers_per_group = -1 is used, then defaults to using the  
            LOCAL_WORLD_SIZE. (Default: -1)
        cache_split_params (bool): cache split parameters across iterations.
            (Default: False)
        max_grad_norm (Optional[float]): maximum gradient norm for gradient
        ...
        use_dtensor (bool): use DTensor. Requires PyTorch 2 nightly. Otherwise,
            uses Tensor. (Default: True)
        debug_mode (bool): debugging mode. Uses more memory to compute error to
            fp64 case. Must enable logging level to DEBUG. (Default: False)
  ...
  def __init__(
    ...
    if not num_trainers_per_group >= -1:
      raise ValueError(
          f"Invalid number of GPUs per group: {num_trainers_per_group}. "
          "Must be >= -1.")
    if isinstance(exponent_override, list):
      if not all(e >= 0 for e in exponent_override):
        raise ValueError(
            f"Invalid exponent override list: {exponent_override}. "
    ...
    if max_grad_norm is not None and not max_grad_norm > 0.0:
      raise ValueError(
          f"Invalid maximum gradient norm for clipping: {max_grad_norm}. "
          "Must be > 0.0.")
    ...
    if num_trainers_per_group > 1 or num_trainers_per_group == -1:
      ...
      if not dist.is_initialized():
        raise ValueError(
            "Using distributed version of Shampoo without initializing "
      ...
      if not dist.get_world_size() >= num_trainers_per_group:
        ...
        logger.warning(
            f"Number of GPUs per group {num_trainers_per_group} is specified "
            f"larger than global world size {dist.get_world_size()}. Setting to"
            " default world size.")
      if not dist.get_world_size() % num_trainers_per_group == 0:
        raise ValueError(
            f"Invalid number of GPUs per group: {num_trainers_per_group}. "
            f"Must divide global world size {dist.get_world_size()}.")
    else:
    ...
    if self._use_nesterov and momentum == 0.0:
      logger.warning(
          "Nesterov flag is enabled but momentum parameter is zero! Continuing "
          "without using momentum or Nesterov acceleration...")

    if start_preconditioning_step == -1:
      self._start_preconditioning_step = precondition_frequency
      logger.warning(
          f"start_preconditioning_step set to -1. Setting "
          "start_preconditioning_step equal to precondition frequency "
          f"{precondition_frequency} by default.")
    elif start_preconditioning_step < precondition_frequency:
      raise ValueError(f"Invalid start_preconditioning_step value: "
                       f"{start_preconditioning_step}. "
                       f"Must be >= {precondition_frequency = }.")
    else:
    ...
    self._optimizer_info = [{
        ...
        NUM_PARAM_ELEMS:
            0,
        PER_ORDER_NUM_PARAM_ELEMS:
            [0] * (self._max_order(self.param_groups[group_idx][PARAMS]) + 1),
        PARAM_NUM_BYTES:
            0,
  ...
  def _initialize_preconditioners_and_steps(
    ...
    for group_idx, (group, buffer_ranks) in enumerate(
      ...
      for idx, p in enumerate(group[PARAMS]):
        ...
        # the max_preconditioner_dim; see features above.
          ...
          state[PRECONDITIONERS] = (
              AdagradPreconditioner(
                  ...
                  use_dtensor=self._use_dtensor,
                  communication_dtype=self._communication_dtype,
              ) if torch.any(dims > self._max_preconditioner_dim) else
        ...
        # preconditioner if dimension is larger than max_preconditioner_dim;
        # see feature above.
          ...
          raise ValueError("Large dim method " + str(self._large_dim_method) +
                           " is not implemented!")
  ...
  def _log_precond_info(
    ...
    precond = preconditioner.get_debug_info()
    precond_info = ', '.join(
        [f'{key}={val}' for key, val in precond.items() if key != 'name'])
    self._optimizer_log += (
        f"{precond['name']}(group_idx={group_idx}, {precond_info})\n")
    ...
    for group_rank in range(self._num_trainers_per_group):
      optimizer_info[PRECONDITIONER_NUM_BYTES][
          group_rank] += preconditioner.get_num_bytes(group_rank=group_rank)

  @torch.no_grad()
  def _log_buffer_rank_info(self,
                            buffer_ranks: List[Tuple[torch.Tensor, int]],
                            group_idx: int):
    optimizer_info = self._optimizer_info[group_idx]
    # Lists of distributed buffer sizes associated with each rank.
    optimizer_info[DIST_BUFFER_SIZE_LIST_PER_RANK] = [[
        buffer.nelement() * buffer.element_size() for buffer,
        rank in buffer_ranks if rank == group_rank
    ] for group_rank in range(self._num_trainers_per_group)]
    ...
    optimizer_info[TOTAL_DIST_BUFFER_SIZE] = (
        max(optimizer_info[LOCAL_DIST_BUFFER_SIZES]) *
        self._num_trainers_per_group)
    # The number of local buffers, which is equal to the number of leaf
    # preconditioners of each rank
...
"""
  ...
  @torch.no_grad()
  def _assign_preconditioners_to_ranks(
      self) -> List[List[Tuple[torch.Tensor, int]]]:
    ...
    # Currently supports only homogeneous architectures and assumes that the
    # environmental variable LOCAL_WORLD_SIZE is set (for example, through
    # torchrun / torch.distributed.launch).
      ...
      logger.info(f"Distributed Shampoo: Global Rank = {dist.get_rank()}, "
    ...
    for group in self.param_groups:
      ...
      # Calculate necessary buffer sizes over all preconditioners for gradient
      # communication.
      buffer_sizes = []
      for p in group[PARAMS]:
        if self._large_dim_method == LargeDimMethod.BLOCKING:
      ...
      buffer_size_ranks = distribute_buffer_sizes(buffer_sizes,
                                                  self._num_trainers_per_group)

      # Allocate a single huge tensor.  Now every rank has the same size of
      # buffer so that we can use all_gather_into_tensor which performs the best
      # in NCCL.
      # TODO: Switch from AllGather to AllGatherV once underlying NCCL supports
      # efficient AllGatherV.
      max_buffer_size_sum = max(
          sum(s
              for s, r in buffer_size_ranks
              if r == group_rank)
          for group_rank in range(self._num_trainers_per_group))
      ...
      # Further split local_dist_buffers so that we can assign them to
      # preconditioners
  ...
  def _compute_and_log_root_inverse_residuals(self,):
    ...
    if self._preconditioner_dtype == torch.float64:
      ...
      logger.warning(
          "Expected relative error/residual not supported for precision lower "
          "than float32.")
    ...
    logger.debug(
        f"Relative Error (||X - X_hat||_inf / ||X||_inf)       "
        f"Average: {torch.mean(relative_errors)}, "
        "Quantiles [0, 25, 50, 75, 100]: "
        f"{torch.quantile(relative_errors, quantiles, interpolation='nearest')}"
    )
    quantiles = torch.quantile(
        relative_residuals, quantiles, interpolation='nearest')
    logger.debug(f"Relative Residual (||X_hat^-r - A||_inf / ||A||_inf) "
                 f"Average: {torch.mean(relative_residuals)}, "
                 f"Quantiles [0, 25, 50, 75, 100]: {quantiles}")

  @torch.no_grad()
  def _update_preconditioners(self):
    """Updates preconditioners.

        Note: If using L2-regularization/weight decay, it is computed within
        this function and therefore should not be recomputed elsewhere.
  ...
  def _clip_gradients(self):
    ...
    clipped_grad_norm = (1.0 if global_grad_norm <= self._max_grad_norm else
                         global_grad_norm / self._max_grad_norm)
  ...
  def _compute_param_norm_rescalings(
      self,
      group: Dict[str, Any],
      split_preconditioned_grad: List[torch.Tensor]) -> List[torch.Tensor]:

    # Instantiate list for split parameter norm rescalings (for LARS and
    # LAMB-style updates).
    ...
    for p in group[PARAMS]:
      ...
      search_direction_norm = (
          torch.cat([(torch.norm(split_preconditioned_grad[i])**2).reshape(1)
                     for i in range(block_idx, block_idx + block_count)
                    ]).sum().sqrt())
      param_rescaling = (
          param_norm / search_direction_norm if param_norm != 0.0 and
          search_direction_norm != 0.0 else torch.ones_like(param_norm))
  ...
  def _iterate_step(self) -> int:
    for group in self.param_groups:
      for p in group[PARAMS]:
        self.state[p][STEP] += 1
    p = next(iter(self.param_groups[0][PARAMS]))
  ...
  def step(self, closure=None):
    ...
    # Computes root inverse of all preconditioners every
    # self._precondition_frequency
    # after the self._start_preconditioning_step iteration.
    if (iteration % self._precondition_frequency == 0 and
        iteration >= self._start_preconditioning_step):
    ...
    for group in self.param_groups:
      ...
      if self._use_decoupled_weight_decay and weight_decay != 0.0:
        ...
        # Compute rescaling factor when using param norm to scale weight decay
        ...
        if self._grafting_type in [GraftingType.LARS, GraftingType.LAMB]:
          ...
          torch._foreach_mul_(split_search_directions,
                              split_param_norm_rescalings)
      # Update momentum.
  ...
  def _flattened_state(self) -> Dict[str, Any]:
    """Retrieve flattened state for loading checkpoint.
        ...
        Returns:
            flattened_dict (Dict[str, Any]): Flattened version of parameter
                states.
  ...
  def state_dict(self):
    raise NotImplementedError(
        "Distributed Shampoo does not support the standard state_dict() method "
        "for checkpointing!")

  def load_state_dict(self, state_dict: Mapping[str, Any]) -> None:
    raise NotImplementedError(
        "Distributed Shampoo does not support the standard load_state_dict "
        "method for checkpointing!")

  def distributed_state_dict(
      self,
      key_to_param: Union[Mapping[str, Parameter],
    ...
    """Distributed state dict based on TorchRec's KeyedOptimizer.
        ...
        This allows for advanced functionality like optimizer re-sharding to be
        implemented.

        Can also handle classes and supported data structures that follow the
        PyTorch stateful protocol.

        Args:
            key_to_param (Union[Mapping[str, Any],
                Iterator[Tuple[str, Parameter]]]): Maps some FQN to the
                parameters in the model. If an iterator, will convert to a
                dictionary.
            save_param_groups (bool): Flag for saving parameter groups.
                (Default: False)

        Returns:
            state_dict (Dict[str, Any]): Dictionary containing the optimizer
                state and potentially parameter groups.
    ...
    if not isinstance(key_to_param, dict):
      key_to_param = dict(key_to_param)
  ...
  def load_distributed_state_dict(
    ...
    """Load state dict based on TorchRec's KeyedOptimizer.
        ...
        it requires implementations to fully initialize their state during first
        optimization iteration, and it prohibits loading an empty state into
        already initialized KeyedOptimizer and vise versa.
        ...
        Because of introduced strictness it allows us to:
            ...
            * avoid state duplication by directly copying into state tensors,
              ...
              # copy state directly into tensors, re-shard if needed
              load_checkpoint(sd)
              optimizer.load_state_dict(sd)  # replace param_groups

        Args:
            state_dict (Dict[str, Any]): State dictionary to load containing the
    ...
    for param_key, param in key_to_param.items():
      ...
      if len(state[param]) != len(new_state[param_key]):
        raise ValueError(f"Different state size: {len(state[param])} vs "
                         f"{len(new_state[param_key])}")
    ...
    if save_param_groups:
      ...
      if len(param_groups) != len(new_param_groups):
        raise ValueError(
            f"Different param_groups count: {len(param_groups)} vs "
            f"{len(new_param_groups)}")
      param_to_key = {param: key for key, param in key_to_param.items()}
      group_map = {}
      for group in param_groups:
        param_keys = []
